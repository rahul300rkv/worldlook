...
World Monitor publishes a public, searchable [Data Source Catalog](https://worldmonitor.app/sources). Its manifest-derived inventory currently lists 547 active providers, 549 upstream hosts, 298 structured endpoints, and 279 news and OSINT feeds. The separate application inventory records 658 configured feed definitions, and the Telegram catalog enables 56 channels. These figures describe different inventory layers and should not be added together.
...
500+ curated news feeds across geopolitics, defense, energy, think tanks, and regional news (Middle East, Africa, Latin America, Asia-Pacific), backed by 549+ observed upstream hosts.
